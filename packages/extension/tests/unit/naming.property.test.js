...
const elementArb = fc.record({
  ...
  role: fc.option(fc.constantFrom('button', 'link', 'textbox', 'option', 'menuitem', null), {
    nil: null,
  }),
  type: fc.option(fc.constantFrom('text', 'password', 'submit', 'checkbox', 'radio', null), {
    nil: null,
  }),
    ...
    await fc.assert(
      fc.asyncProperty(contextLifecycleArb, async (action) => {
        if (action.type === 'context_switch') {
          ...
          assert.ok('source' in action, 'context_switch must have a "source" field');
          assert.ok(
            !('url' in action),
        ...
        if (action.type === 'context_open') {
          ...
          assert.ok('source' in action, 'context_open must have a "source" field');
          assert.ok(!('url' in action), 'context_open must not have a "url" field — use "source"');
        }
