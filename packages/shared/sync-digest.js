 ...
 */
// Governance declared in scripts/area-map.json (see its declared-governance entry): the digest's identity projection is defined over the format's committed step-history model (logical_id/uuid versioning, tombstones, re-records), so changes to that model and this projection move together; the per-platform schemas are authoritative for field semantics.
