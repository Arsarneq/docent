 ...
 * version number stay outside the gate while Markdown emphasis around a real
 * citation does not hide it, and the lock-surface cases prove the surfaces are
 * held to each other on every audit — a retired entry keeping its number and
 * losing its title, the clause document's own prose held to its own list, an
 * empty parse named rather than passed vacuously. Which bucket a lock red lands
 * in is asserted, not incidental: a surface fact is the surfaces' to answer for,
 * and only a row's own citation is the registry's.
 *
 * Because the lock legs are unconditional, every fixture carries the lock
 * surfaces (see {@link LOCK_DOC_TEXT}); a case that means to red them says so.
 */
...
/**
 * The tracked universe every fixture starts from: the check's own enumerated
 * citable root files — held against the tracked set on every audit, cited or
 * not — plus the fixture doc, the check it names, and the two lock surfaces.
 */
const BASE_FILES = [
  ...CITABLE_ROOT_FILES,
  'docs/testable.md',
...
/** The contents every fixture reads, before a case overrides them. */
const BASE_CONTENTS = {
...
describe('extractCitedTargets', () => {
  ...
  it('drops a glob whole, leaving no fragment to resolve', () => {
  ...
  it('drops a brace alternation whole, leaving no fragment to resolve', () => {
...
describe('auditClauseRegistry — citation resolvability', () => {
  ...
  it('leaves a glob outside the gate, whole rather than in fragments', () => {
...
describe('auditClauseRegistry: cited paths across the tree', () => {
  ...
  function auditOne(clause, extraFiles = []) {
    ...
    return auditClauseRegistry({
      ...
      files: [...CITABLE_ROOT_FILES, 'docs/t.md', LOCK_DOC, LOCK_SUITE_PATH, ...extraFiles],
...
describe("auditClauseRegistry — the check's own registered list", () => {
  ...
  it('reports nothing when every enumerated entry is tracked', () => {
...
describe('auditClauseRegistry — hygiene-lock surfaces', () => {
  ...
  function lockAudit({
    ...
    const contents = { [LOCK_DOC]: docText ?? lockDocWith(entries) };
    if (!omitSuite) contents[LOCK_SUITE_PATH] = suiteText ?? lockSuite(titles);
    return auditClauseRegistry({
      registry,
      files: [...CITABLE_ROOT_FILES, LOCK_DOC, LOCK_SUITE_PATH],
      readFile: (f) => contents[f] ?? null,
      packageScripts: ['test:shared'],
    });
  }
