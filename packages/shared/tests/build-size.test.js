...
const extensionExcludes = ['node_modules', 'tests', '.git'];
...
describe('Build size: Desktop dist', () => {
  it('total JS size is under 250KB', () => {
    ...
    assert.ok(size < 250 * 1024,
      `Desktop dist JS is ${formatSize(size)} (limit: 250KB).`);
  });
  ...
  it('total dist size is under 1MB', () => {
    ...
    assert.ok(size < 1024 * 1024,
      `Desktop dist total is ${formatSize(size)} (limit: 1MB).`);
  });
});
