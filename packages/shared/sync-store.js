 ...
 * `save_state` JSON blob (R10.1, R16.1, R17.1).
 *
 ...
 * complete, keeping the save/load round-trip stable (R10.1, design Property 5).
 *
 ...
 *
 * Design references:
 *   - R10.1  — conflict / review state is persisted durably across restarts.
 *   - R16.1  — all conflict-handling state is client-side, in one shared shape.
 *   - R17.1  — detection / snapshot / deferral / resolution state lives here.
 *
 ...
 * believing a Connection_Test passed (R23.2).
 *
 ...
 * Connection_Test passes (R22.1, R22.2, R23.1).
 *
 ...
 * with its documented default otherwise, so a pre-R1 blob omitting `settings`
 ...
 * documented empty defaults (R4.9, R22.1, R23.1).
 *
 ...
 * otherwise. `dismissedIncoming` and `settings` (added in Revision R1) are
 * normalized the same way, so a pre-R1 persisted blob that omits them still
 * loads cleanly with `dismissedIncoming` empty and `settings` at the documented
 * defaults (R10.1, R22.1, R23.1). Only the recognized top-level keys are kept,
 ...
 * always receive every keyed map (R10.1, R16.1, R17.1).
 *
 ...
 * afterwards (R5.2, R9.5). Matches the clone strategy used by sync-baseline.js.
 *
 ...
 * abort without having lost or half-written any version (R5.3, R5.5).
 *
 * @template T
 * @param {T} value
 * @returns {T}
 ...
 * (R5.1, R5.2). Idempotent: because the record is keyed by `unitRef`, re-detecting
 * the same diverged Unit across repeated sync cycles keeps exactly ONE Conflict
 * record rather than accumulating duplicates (R10.3); the recoverable copies are
 ...
 *
 * Mutual exclusion (R10.3): a Unit is either in Conflict, in Review-and-Accept,
 * or NONE. Recording a Conflict therefore removes any Review item for the same
 * `unitRef`.
 *
 * Atomicity (R5.3, R5.5): both deep copies are built BEFORE the store is mutated,
 ...
 * @param {import('./sync-types.js').UnitCopy} localVer - the local version to retain (R5.2)
 * @param {import('./sync-types.js').UnitCopy} incomingVer - the incoming version to retain (R5.2)
...
export function upsertConflict(state, unitRef, localVer, incomingVer, now = Date.now) {
  ...
  // mutation, leaving the store unchanged (R5.3, R5.5).
  ...
  // Mutual exclusion: a Unit cannot be in Review and Conflict at once (R10.3).
 ...
 * (R4.1, R9.5). Idempotent: keyed by `unitRef`, re-detecting the same
 * changed-incoming Unit keeps exactly ONE Review record (R10.3); the incoming
 ...
 * workflow, R4.5).
 *
 * Mutual exclusion (R10.3): recording a Review item removes any Conflict for the
 ...
 * @param {import('./sync-types.js').UnitRef} unitRef - the idempotency key
 * @param {import('./sync-types.js').UnitCopy} incomingVer - the incoming version to retain (R4.6)
 * @param {() => number} [now=Date.now] - clock source (injectable for tests);
 ...
 *
 * Called when resolution of a Unit completes (R12.5). Because `reviews` and
 ...
 * later sync cycle processes it normally rather than as a duplicate (R10.4).
 *
 ...
 * silently swallowed by a leftover dismissal (R12.5, R10.4).
 *
 * @param {import('./sync-types.js').SyncState} state - the loaded SyncState;
 ...
 * Unit normally instead of treating it as a duplicate of a prior record (R10.4).
...
// ─── Decline-dismissal of a declined incoming version (R4.9, R4.10) ───────────
 ...
 * re-offer it as a fresh Review-and-Accept item (R4.9).
 *
 ...
 * classified afresh (R4.10, enforced together with {@link isDismissedIncoming}).
 * Declining keeps local data untouched and never pushes — it is purely a "do not
 * re-offer this version" marker (R4.8).
 *
 * @param {import('./sync-types.js').SyncState} state - the loaded SyncState;
 ...
 * left for the detector to classify afresh (R4.9, R4.10).
 *
 * @param {import('./sync-types.js').SyncState} state - the loaded SyncState
 * @param {import('./sync-types.js').UnitRef} unitRef
...
// ─── Client-local reconciliation-policy + Auto-Sync settings (R22, R23) ───────

/**
 * Read the client-local {@link import('./sync-types.js').ReconciliationSettings},
 * normalized into the full shape. A state that predates Revision R1 (or was
 ...
 * object (R22.1, R22.2, R23.1). The returned object is an independent copy;
 ...
 * never transmitted to the Sync_Server (R22.3, R23.1, R23.3).
 *
 * @param {import('./sync-types.js').SyncState} state - the loaded SyncState;
