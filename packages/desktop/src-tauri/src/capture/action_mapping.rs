...
//
// Governance declared in scripts/area-map.json (see its declared-governance entry): the actions mapped here are .docent.json fields; the per-platform schemas are authoritative for field semantics.
