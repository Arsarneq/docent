 ...
 * driven end-to-end over HTTP through the test harness.
 *
 ...
 *   - reset: empties the store → a subsequent GET /projects returns `[]`,
 *     and the response is `{ ok: true, cleared: <n> }`.
 *   - dump: returns `count` + per-project `last_modified` + `etag` WITHOUT
 *     mutating the store; the dump `etag` matches the `ETag` header from
 *     GET /projects/:id.
 *   - seed with caller payloads: stores each payload without a client PUT;
 *     response `{ ok: true, seeded: <n> }`; observable via the manifest and reads.
 *   - seed with `{ samples: true }`: stores BOTH an `extension`- and
 *     a `desktop-windows`-stamped sample, content-equivalent to the bundled files.
 *   - seed with invalid JSON: 400, store unchanged.
 *
...
describe('debug: reset — POST /__debug/reset', () => {
  let server;
  before(async () => {
...
describe('debug: dump — GET /__debug/dump', () => {
  let server;
  before(async () => {
    server = await startTestServer();
  });
  ...
  it('returns count + per-project last_modified + etag without mutating the store', async () => {
    ...
    // count matches the number of stored projects.
    assert.equal(dump.body.count, 2);
    assert.equal(dump.body.projects.length, 2);

    // Each entry carries project_id, name, last_modified, and etag.
    ...
    // count still matches the stored projects.
...
describe('debug: seed with caller payloads — POST /__debug/seed', () => {
  let server;
  before(async () => {
    server = await startTestServer();
  });
  ...
  it('stores each payload without a client PUT and reports the count seeded', async () => {
    ...
    // Observable via the manifest: one entry per seeded project.
    const manifest = await request(server.baseUrl, 'GET', '/projects');
    assert.equal(manifest.status, 200);
    const ids = manifest.body.map((e) => e.project_id).sort();
    assert.deepEqual(ids, ['seed-a', 'seed-b']);

    // And via GET /projects/:id: each payload is stored verbatim.
...
describe('debug: seed with { samples: true } — POST /__debug/seed', () => {
  let server;
  before(async () => {
    server = await startTestServer();
  });
  ...
  it('stores both an extension- and a desktop-windows-stamped project, content-equivalent to the samples', async () => {
    ...
    // Read back content-equivalent to the bundled sample files — the
...
describe('debug: seed with invalid JSON — POST /__debug/seed', () => {
  let server;
  before(async () => {
    server = await startTestServer();
  });
  ...
  it('responds 400 and leaves stored data unchanged', async () => {
    ...
    // A malformed body (sent as-is by the harness) is rejected with 400.
    const seed = await request(server.baseUrl, 'POST', '/__debug/seed', {
      body: '{ not valid json',
    });
