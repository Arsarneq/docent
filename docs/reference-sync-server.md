...
[Docent Sync Protocol](sync-protocol.md). It exists as a faithful, easy-to-read
...
The [Sync Protocol](sync-protocol.md) is the authoritative contract — it
...
[Sync Protocol](sync-protocol.md) exactly as specified there. The request and
...
Returns the [project manifest](sync-protocol.md#get-projects): a JSON array of
...
Returns the stored [`Full_Project_Payload`](sync-protocol.md#get-projectsid)
...
[protocol contract](sync-protocol.md#put-projectsid). The path `:id` must match
...
[protocol's last-write-wins window](sync-protocol.md#known-limitation-the-last-write-wins-window)
describes.
...
- [Sync Protocol](sync-protocol.md) — the authoritative protocol contract this
  server implements.
- [Session Format](session-format.md) — the `.docent.json` specification that
