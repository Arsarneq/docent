...
missing from the table is a bug in this guide — and the doc-closure gate
...
| Doc closure           | `lint`                                        | A guides closure claim disagrees with the tree: the workflow-inventory table and `.github/workflows/`, the act table and `test.yml`'s job ids, this table and the `lint:*` family plus the `lint` job's `npm run` steps, or an `npm run` command cited in tracked markdown that no tracked package.json defines ([`check-doc-closure.js`](../../scripts/check-doc-closure.js)). | `npm run lint:doc-closure`                                                |
