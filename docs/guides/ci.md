...
| Schema echoes         | `lint`                                        | An echo of the schemas composed from their source layers drifts from them: a document that restates the schemas' authority over the prose no longer states it; an object subschema's `additionalProperties` posture is not the one its class states, an exempted def has lost the shape its exemption rests on, or the action wrapper and the defs carrying the action prefix no longer select each other; a session-format field table disagrees with the composed def it describes (its field names, their required status, or the `one of` marking), the two platforms disagree on a def they share, or the document's field tables and the check's registers fall out of agreement either way ([`check-schema-echo.js`](../../scripts/check-schema-echo.js)). | `npm run lint:schema-echo`                                                |
