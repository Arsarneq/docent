...
| Verification inventories | `lint`                                        | A verification document's inventory and what it describes fall out of one-to-one agreement: the relaxation kinds or their covered fields against the comparator's constants, the normalization classes against its class map, the desktop session catalogue against `corpus/manifest.json`, or the predicate tables against the lint's catalogue. The job cites are held one-way instead — a cited job that `test.yml` does not define is red, while a job no document cites is fine. The gate also reds when an inventory is no longer where the check reads it: an unreadable list item, table cell, or session token, a header tuple matching anything but exactly one table, or a scanned document that cites no job at all ([`check-verification-inventory.js`](../../scripts/check-verification-inventory.js)).                                                                                                   | `npm run lint:verification-inventory`                                     |
