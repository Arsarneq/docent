...
| Test inventories      | `lint`                                        | A suite document's inventory tables and the suite they enumerate fall out of one-to-one agreement, a `TRACKED_FILES` entry does not identify one tracked source, or an inventory is no longer where the check reads it ([`check-test-inventory.js`](../../scripts/check-test-inventory.js)).           | `npm run lint:test-inventory`                                             |
