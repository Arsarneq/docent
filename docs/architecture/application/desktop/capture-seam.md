...
| `stop()`                | Stop observing. Held completed actions are flushed before teardown — a bounded detach with in-place rescue of a worker wedged in an unresponsive accessibility call (the model is [DCP-12](windows/capture-principles.md#worker-delivery-guarantees)).                                                               |
