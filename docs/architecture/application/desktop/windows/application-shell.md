...
| `commit_barrier`             | JS → Rust | Runs the step-commit flush barrier mid-capture ([DCP-2](capture-principles.md#architecture)) and returns `{ barrier_id, wedged_workers, completion }`; the completion sentinel it triggers arrives on `capture:action`.                                                                                                                                                                                                                                                         | The adapter's `commitWithCompleteness` — the panel's step commit when capture is already stopped (a commit while recording flushes through `stop_capture` instead). |
