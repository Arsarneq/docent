...
| `stop_capture`               | JS → Rust | Stops the capture layer; runs the in-order commit flush barrier as it stops — draining held completed actions behind the step and emitting the `barrier_complete` sentinel last — before teardown, and returns `{ barrier_id, wedged_workers }` ([DCP-2](capture-principles.md#architecture), [DCP-12](capture-principles.md#worker-delivery-guarantees)). | `panel.js` recording stop paths; the adapter's `stopWithCompleteness`, invoked by the panel's step commit while recording.                                          |
...
| `commit_barrier`             | JS → Rust | Runs the step-commit flush barrier mid-capture ([DCP-2](capture-principles.md#architecture)) and returns `{ barrier_id, wedged_workers }`; the completion sentinel it triggers arrives on `capture:action`.                                                                                                                                                | The adapter's `commitWithCompleteness` — the panel's step commit when capture is already stopped (a commit while recording flushes through `stop_capture` instead). |
