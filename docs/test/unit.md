...
| `check-adapter-surface.test.js`                       | The adapter-seam admission test: both held directions between the typedef and each concrete adapter's default-exported literal, the platform-specific member it admits, the decoy literals beside it, and the extractors' refusals. |
...
| `check-extension-surface.test.js`                     | The extension-surface admission test: the manifest's permissions, the worker dispatcher, and the panel's literal sends against their documented enumerations.                                                                       |
...
| `check-test-inventory.test.js`                        | The test-inventory lint: the inventory, coverage-list, and kill-set red paths, the mutate-scope agreement and dead modules, the registration closure, the undiscovered-binary admission by either route, and the real-tree locks.   |
