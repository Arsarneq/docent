...
| `recorder-coverage.spec.js`   | Recorder edge paths: right-click target variants, arrow keys on listboxes and sliders, `select` capture, Tab↔focus correlation (with click-focus dedup and the password-field focus skip), stop/restart recording transitions and the idle-surface negatives of [ECP-2](../architecture/application/extension/capture-principles.md) (no recorder injected into a document or a tab that loads while no recording runs; a recorder left in a still-open document attempting no append after the stop — each paired in-test with its positive control), blur-on-submit change suppression, body/html click rejection, the double-injection guard, and contenteditable debounce plus blur flush. |
