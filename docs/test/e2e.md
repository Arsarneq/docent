...
| `registry-departures.spec.js`      | Registry departure routes of [ECP-3](../architecture/application/extension/capture-principles.md#frame-trust-and-readiness) beyond the subframe drop `recorder-coverage.spec.js` pins, plus the arrival-side lazy reseed, with suspension loss simulated and labeled, through the introspection handle [extension runtime ERT-5](../architecture/application/extension/runtime.md#lifecycle-and-the-persisted-state-model) states; the spec header carries the per-route detail. The programmatic-tab set legs exercise both conjuncts of [ECP-12](../architecture/application/extension/capture-principles.md#exceptions-within-the-surface)'s `window.close()` suppression, over the set ERT-1 places in the correlation-marker class: with the close held inside the timing window the membership is varied — planted membership suppresses the context_close proxy for a tab closed after a user action, and for a tab the worker classified itself that organic membership suppresses the proxy for a scripted close, while a wiped set — the loss signature — lets the same scripted close append a context_close the user never performed; and with the membership held planted the window is varied, a close landing past the recent-action window appending the proxy that same membership suppresses inside it. A further case pins the platform premise the same-value clear routes' call-site attributions rest on: a write of the value a key already holds fires no change event, so a route writing the recording flag the value it already holds leaves the flag watch idle and its own clear the only one — pinned on a key the extension does not own. |
